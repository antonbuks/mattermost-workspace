...
cat config/config.json | \
    jq '.SqlSettings.DataSource = "postgres://mmuser:mostest@localhost/migrated?sslmode=disable&connect_timeout=10"'| \
...
cat config/config.json | \
    jq '.SqlSettings.DataSource = "postgres://mmuser:mostest@localhost/latest?sslmode=disable&connect_timeout=10"'| \
    jq '.SqlSettings.DriverName = "postgres"' > $TMPDIR/config.json
