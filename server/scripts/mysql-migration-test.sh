...
cat config/config.json | \
    jq '.SqlSettings.DataSource = "postgres:postgres@tcp(localhost:3306)/migrated?charset=utf8mb4,utf8&readTimeout=30s&writeTimeout=30s"' | \
...
cat config/config.json | \
    jq '.SqlSettings.DataSource = "postgres:postgres@tcp(localhost:3306)/latest?charset=utf8mb4,utf8&readTimeout=30s&writeTimeout=30s"' | \
    jq '.SqlSettings.DriverName = "mysql"' > $TMPDIR/config.json
